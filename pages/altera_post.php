...
<head>
  ...
  <title>Blog From Hell - Atualiza Post</title>
</head>
...
<body>
  ...
  <div class="container form-post">
    <h1 class="title blue"><span>&#62;</span>Atualize as informações do post<span>&#46;</span></h1>
    <div class="form grid-8">
      <form id="new_post" class="general-form" method="POST" enctype="multipart/form-data">
        <label for="titulo">Título:</label><input type="text" value="<?=$titulo?>" name="titulo" id="titulo" placeholder="Título da Postagem">
        <label for="resumo">Resumo: </label><input type="text" value="<?=$resumo?>" name="resumo" id="resumo" placeholder="Resumo do post...">
        <label for="postagem">Postagem: </label><textarea name="postagem" id="postagem" cols="50" rows="8"
          placeholder="Insira o post aqui."><?=$texto?></textarea>
        <label for="imagem">Imagem: </label><input type="file" name="imagem" id="imagem">
        <label for="posicao">Posição: </label><input type="text" value="<?=$posicao?>" name="posicao" id="posicao" placeholder="Insira onde a imagem será posicionada.">
        <input type="hidden" name="codigo" value="<?=$codigo?>">
        <button type="submit" name="edita" class="submit-button">Salvar</button>
      </form>
    </div>  
  </div>
